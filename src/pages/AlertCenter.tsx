...
export default function AlertCenter() {
  ...
  const handleApprove = (alert: AlertItem) => {
    ...
    const record: ApprovalRecord = {
      ...
      time: now,
      comment: `${stepConfig.handler}${stepConfig.action}通过`,
    }
    ...
    <div className="space-y-5 font-noto">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filteredAlerts.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-gray-500">暂无预警数据</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {selectedAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setSelectedAlert(null)}>
          <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-surface-border bg-surface-card p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-6 grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <span className={selectedAlert.level === 1 ? 'text-alert-orange' : 'text-alert-red'}>{selectedAlert.level}级预警</span>
              </div>
              <div>
                  ...
                  return (
                    <div key={s.step} className="flex flex-1 items-start">
                      <div className="flex flex-col items-center">
                        <div className={cn('flex h-8 w-8 items-center justify-center rounded-full border-2', isCompleted && 'border-green-500 bg-green-500/20', isCurrent && 'border-carbon-500 bg-carbon-500/20', isPending && 'border-gray-600 bg-gray-600/20')}>
                          {isCompleted ? <CheckCircle className="h-4 w-4 text-green-500" /> : isCurrent ? <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-carbon-500" /> : <Clock className="h-4 w-4 text-gray-600" />}
                        </div>
                        <span className={cn('mt-1 text-xs', isCompleted ? 'text-green-400' : isCurrent ? 'text-carbon-400' : 'text-gray-600')}>{s.role}</span>
                        {isCompleted && record ? (
                          <>
                            <span className="flex items-center gap-1 text-[10px] text-green-300"><User className="h-2.5 w-2.5" />{s.handler}</span>
                            <span className="text-[10px] text-gray-500">{record.time}</span>
                          </>
                        ) : isCurrent ? (
                          <span className="flex items-center gap-1 animate-pulse text-[10px] text-carbon-400"><User className="h-2.5 w-2.5" />等待 {s.handler} 处理</span>
                        ) : (
                          <span className="flex items-center gap-1 text-[10px] text-gray-600"><User className="h-2.5 w-2.5" />{s.handler}</span>
                        )}
                      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-surface-border text-gray-400">
                      ...
                      <th className="px-2 py-2 text-left font-medium">负责人</th>
            ...
            {canApprove(selectedAlert) && (
              <button onClick={() => handleApprove(selectedAlert)} className="w-full rounded-lg bg-carbon-500 py-2.5 text-sm font-medium text-surface-dark hover:bg-carbon-400 transition-colors">
